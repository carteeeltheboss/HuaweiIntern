...
\begin{minipage}[c]{0.1\textwidth}
    \raggedleft
    \href{https://github.com/carteeeltheboss/HuaweiIntern}{\includegraphics[width=1.5cm]{qr1.png}}
\end{minipage}
...
\section*{12. MLOps: Data Versioning with DVC}
\begin{itemize}
    \item \textbf{Objective:} Implement robust data versioning alongside Git to track dataset changes efficiently.
    \item \textbf{Implementation:} Initialized DVC in the repository and tracked a baseline dataset using the classic Iris dataset: \texttt{X, y = load\_iris(return\_X\_y=True)}. This approach ensures that data and model states are versioned symmetrically with code commits.
\end{itemize}

\section*{13. Experiment Tracking with MLflow}
\begin{itemize}
    \item \textbf{Objective:} Track model hyperparameters, performance metrics, and artifacts to compare different training iterations.
    \item \textbf{Execution:} Trained a \texttt{RandomForestClassifier(n\_estimators=estimators, max\_depth=depth)} on the DVC-tracked dataset.
    \item \textbf{Analysis:} Utilized the \texttt{mlflow ui} to visualize the runs. By modifying parameters and comparing every run directly through the web interface, we gained a clear understanding of MLflow's execution logging and visualization capabilities.
\end{itemize}
...
\begin{figure}[h!]
    ...
    \caption{\small MLflow UI comparing the RandomForest runs}
\end{figure}
...
\section*{14. Model Monitoring: Understanding Data Drift}
Data drift is a critical issue in production ML systems that eventually leads to model drift and performance degradation. To address this, a theoretical presentation was created to define the problem, accompanied by a progressive, constructive Jupyter notebook explicitly demonstrating the issue in a real-world scenario.

\subsection*{Synthetic Dataset: The Credit Risk Scenario}
We generated realistic synthetic data simulating a \textbf{credit default prediction} problem to practically analyze drift.

\textbf{Features:}
\begin{itemize}
    \item \texttt{age} --- borrower age (continuous)
    \item \texttt{income} --- annual income in USD, log-normal (continuous)
    \item \texttt{credit\_score} --- FICO score 300--850 (continuous)
    \item \texttt{loan\_amount} --- requested loan amount (continuous)
    \item \texttt{employ\_years} --- years at current employer (continuous)
    \item \texttt{region} --- geographic region (categorical)
\end{itemize}

\textbf{Target:} \texttt{default} --- 1 if the borrower defaulted, 0 otherwise.

\textbf{Default probability formula (true DGP):}
...
We generated 4 dataset variants to simulate different drift types, analyzed their representations, and compared the specific drift categories:
...
\begin{tabular}{|>{\ttfamily}l|l|l|}
    \hline
    \normalfont\textbf{Mode} & \textbf{What changes} & \textbf{What stays same} \\
    \hline
    reference & --- & baseline \\
    covariate & $P(X)$ & $P(Y|X)$ \\
    label     & $P(Y)$ & $P(X|Y)$ \\
